...
const DosenAddContext = createContext()
export const Add = () => {
    ...
    const formValidation = () => {
        ...
        if (data.foto === null) newError['foto'] = `*Mohon masukkan Foto`;
        ...
        if (data.user.password === '') newError['user.password'] = `*Mohon masukkan Password`;
        if (data.user.password_confirmation !== data.user.password) newError['user.password_confirmation'] = `*Mohon konfirmasi Password dengan benar`;
        if (data.anak !== null) {
    ...
    return (
        <DosenAddContext.Provider value={{ data, setData, errors, setError }}>
        ...
        </DosenAddContext.Provider>
    );
}

const InputFoto = () => {
    const { data, setData, errors, setError } = useContext(DosenAddContext);
    ...
    const handleFileChange = (event) => {
        ...
        if (!status) {
            setData('foto', null);
            setDisplayImage("/storage/foto-user/default.png");
            return;
        }
    ...
    return (
        <div className="mb-8 w-full flex flex-col justify-center gap-5">
            ...
            <div className="flex justify-center">
                <button onClick={handleButtonClick} type="button" className="btn btn-info btn-sm text-white">{data.foto === null ? 'Unggah Foto' : 'Ganti Foto'}</button>
            </div>
            <div className="flex justify-center">
...
const InputStatus = () => {
    const { data, setData, errors } = useContext(DosenAddContext);
        ...
        <div className="mt-5">
            ...
            <div className="mb-4 grid grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <span className={errors['user.password'] ? "text-red-500 text-sm" : "hidden"}>{errors['user.password']}</span>
                </div>
                <div>
                    <input
                        type="text"
                    ...
                    <span className={errors['user.password_confirmation'] ? "text-red-500 text-sm" : "hidden"}>{errors['user.password_confirmation']}</span>
                </div>
            </div>
        </div>
    )
